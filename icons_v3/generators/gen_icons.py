...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "icons", "ch1-3")
ICONS = []
...
# simple: the statement shape itself. A plain statement is nothing but a subject and a predicate.
icon("simple", "plainly says something about something: the statement shape itself (סתם)",
     tile(-9.6, 9.6, -3.6, 3.6, V), V)
...
# comparative: a known matter and an unknown one equated to it (Eng p40, Heb p39: ענין, not a term).
# Kept at unit level on purpose: the text does not say which term the two matters share.
