...
# swaps: recycle-logo arrows. Each tag arrow is a thick band with a sharp fold: a long run,
# then a turn, then the head. Two of them chase each other around the box.
def bent_tile(c, x_tail=-8.2, x_turn=6.4, y_run=-6.9, t=2.1, y_head_base=-0.6, head_len=3.2, barb=1.05,
              single="square", inverted=False, glyph_s=2.8, rotate=False):
    """Top arrow: runs right along y_run (band half-width t), folds down at x_turn, head points down.
    rotate=True turns it 180 degrees into the returning arrow."""
    yo, yi = y_run - t, y_run + t            # outer (top) and inner (bottom) edges of the run
    xo, xi = x_turn + t, x_turn - t          # outer (right) and inner (left) edges of the drop
    tip = y_head_base + head_len
    path = (f'M {x_tail} {yo:.2f} H {xo:.2f} V {y_head_base:.2f} '
            f'L {xo + barb:.2f} {y_head_base:.2f} L {x_turn:.2f} {tip:.2f} L {xi - barb:.2f} {y_head_base:.2f} '
            f'L {xi:.2f} {y_head_base:.2f} V {yi:.2f} H {x_tail} Z')
    gx = (x_tail + xi) / 2
...
icon("converse-limited", "trade places, but it only comes back for some: a shorter, thinner return (חלוף קצתי)",
     bent_tile(V, single="square") + "\n  " +
     bent_tile(V, single="circle", rotate=True, x_tail=-8.2, x_turn=-0.4, t=1.6, y_run=-7.2, y_head_base=-1.6, head_len=2.6, barb=0.8, glyph_s=2.0), V)
