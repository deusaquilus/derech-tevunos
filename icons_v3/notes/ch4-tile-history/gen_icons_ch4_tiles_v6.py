...
# swaps: the same tag arrow with a gentle bend, so two of them suggest the original circular swap
# without closing into a ring. Large radius, off-centre, so each arrow is mostly straight.
def P(r, deg, cx=0.0, cy=0.0):
    a = math.radians(deg)
    return f"{cx + r*math.cos(a):.2f} {cy + r*math.sin(a):.2f}"

def arc_tile(a0, a1, a_tip, c, cx=0.0, cy=0.0, rin=11.2, rout=15.2, barb=1.0, single="square", inverted=False, glyph_s=2.8):
    """A gently curved tag arrow: band from angle a0 to a1 (clockwise on screen) about (cx, cy), head tip at a_tip."""
    rmid = (rin + rout) / 2
    large = 1 if (a1 - a0) > 180 else 0
    path = (f'M {P(rout, a0, cx, cy)} A {rout} {rout} 0 {large} 1 {P(rout, a1, cx, cy)} '
            f'L {P(rout + barb, a1, cx, cy)} L {P(rmid, a_tip, cx, cy)} L {P(rin - barb, a1, cx, cy)} '
            f'L {P(rin, a1, cx, cy)} A {rin} {rin} 0 {large} 0 {P(rin, a0, cx, cy)} Z')
    am = (a0 + a1) / 2
    gx, gy = cx + rmid * math.cos(math.radians(am)), cy + rmid * math.sin(math.radians(am))
    return (f'<path d="{path}" {fill_attrs(c, inverted)}/>\n  ' +
            glyph(single, gx, gy, c, glyph_s, white=inverted))

# top arrow arches over the top (centre below the box); bottom arrow sags under it (centre above)
TOP = dict(a0=234, a1=290, a_tip=303, cx=0.0, cy=8.4)
BOT = dict(a0=54, a1=110, a_tip=123, cx=0.0, cy=-8.4)
...
icon("converse-limited", "trade places, but it only comes back for some: a shorter, thinner return (חלוף קצתי)",
     arc_tile(**TOP, c=V, single="square") + "\n  " +
     arc_tile(54, 86, 99, c=V, cx=0.0, cy=-8.4, rin=11.8, rout=14.6, barb=0.8, single="circle", glyph_s=2.0), V)
