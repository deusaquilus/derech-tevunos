"""Part 2: chapter 4-7 icon candidates. Same 24x24 format as Part 1.
...
icon("equivalent", "the same thing, said twice (דומים)",
     f'<rect x="-3.2" y="-8.6" width="11.8" height="11.8" rx="1.9" {tint(V)}/>\n  '
     f'<rect x="-8.6" y="-3.2" width="11.8" height="11.8" rx="1.9" fill="#ffffff" stroke="{V}" stroke-width="1.45" stroke-linejoin="round"/>\n  '
     f'<circle cx="-2.7" cy="2.7" r="2.0" fill="{V}"/>\n  <circle cx="2.7" cy="-2.7" r="2.0" fill="{V}" fill-opacity="0.5"/>', V)

icon("variant", "share a part, differ in a part (מתחלפים)",
     f'<circle cx="-3.5" cy="0" r="5.9" {tint(V)}/>\n  <circle cx="3.5" cy="0" r="5.9" {tint(V)}/>\n  '
     f'<path d="M 0 -4.75 A 5.9 5.9 0 0 1 0 4.75 A 5.9 5.9 0 0 1 0 -4.75 Z" fill="{V}" fill-opacity="0.55"/>', V)

icon("diametrically-opposed", "head-on: yes versus no, same scope (הפכיים ממש)",
     f'<path d="M -9.2 0 H -2.8" {st(V, 2.7)}/>\n  ' + arrowhead(-1.4, 0, 0, V, 3.0, 2.7) + "\n  " +
     f'<path d="M 9.2 0 H 2.8" {st(V, 2.7)}/>\n  ' + arrowhead(1.4, 0, 180, V, 3.0, 2.7), V)

icon("contradictory", "a general claim against a particular one (מתנגדים)",
     f'<path d="M -9.4 0 H -2.8" {st(V, 3.1)}/>\n  ' + arrowhead(-1.4, 0, 0, V, 3.2, 3.1) + "\n  " +
     f'<path d="M 6.4 0 H 2.9" {st(V, 1.6)}/>\n  ' + arrowhead(1.7, 0, 180, V, 2.0, 1.6), V)

icon("converse", "subject and predicate swap places (חלוף כולל)", swap(V), V)

icon("converse-limited", "swapped, but it only comes back for some (חלוף קצתי)", swap(V, dashed_return=True), V)

icon("contrapositive", "swapped and both flipped to no (חלוף הפכי)",
     f'<rect x="-9.4" y="-9.4" width="18.8" height="18.8" rx="3.2" fill="{V}"/>\n  ' + swap(V, w=1.9, stroke_override="#ffffff"), V)

icon("obverse", "both flipped to their opposites, same picture (מתהפכים)",
     sq(-9.2, -4.2, 8.4, V, filled=True) + f'\n  <circle cx="-5" cy="0" r="2.1" fill="#ffffff"/>\n  ' +
     sq(0.8, -4.2, 8.4, V) + f'\n  <circle cx="5" cy="0" r="2.1" fill="{V}"/>', V)

icon("incongruent", "nothing in common (נבדלים)",
     f'<circle cx="-5.6" cy="0" r="3.6" {tint(V)}/>\n  ' + sq(2.2, -3.4, 6.8, V), V)
...
def link(cx, cy, c, dashed=False):
    extra = ' stroke-dasharray="2 1.6"' if dashed else ""
    return (f'<rect x="{cx-3.1}" y="{cy-5.4}" width="6.2" height="10.8" rx="3.1" {st(c, 1.8)}{extra} '
            f'transform="rotate(-45 {cx} {cy})"/>')

icon("inference-necessary", "cannot accept the one and reject the other (מוכרח)",
     link(-2.5, 2.5, V) + "\n  " + link(2.5, -2.5, V), V)

icon("inference-loose", "suggested by the wording, but separable (בלתי מוכרח)",
     link(-4.4, 4.4, V) + "\n  " + link(4.4, -4.4, V, dashed=True), V)

icon("absolute-opposite", "some do, so some do not (הפך)",
     f'<circle cx="-3.9" cy="-3.9" r="2.9" fill="{V}"/>\n  <circle cx="3.9" cy="-3.9" r="2.9" fill="{V}"/>\n  '
     f'<path d="M -6.6 3.9 H -1.2 M 1.2 3.9 H 6.6" {st(V, 2.4)}/>', V)
...
icon("classical-syllogism", "true of the whole kind, so true of this member (הקש מופתי)",
     f'<circle cx="0" cy="-6.2" r="2.5" fill="{T}"/>\n  '
     f'<path d="M 0 -3.7 V -0.8 M -5.6 -0.8 H 5.6 M -5.6 -0.8 V 2.6 M 5.6 -0.8 V 2.6" {st(T, 1.6)}/>\n  '
     f'<circle cx="-5.6" cy="5.4" r="2.5" fill="{T}"/>\n  <circle cx="5.6" cy="5.4" r="2.5" fill="#ffffff" stroke="{T}" stroke-width="1.45"/>', T)
