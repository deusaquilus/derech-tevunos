...
# swaps: the exchange glyph built from two tag arrows, one going, one coming back.
# In the tag arrow, direction IS subject-to-predicate order, so reversing it IS the swap.
GO = dict(xl=-7.4, xr=9.6, y0=-8.4, y1=-1.4)          # top arrow, points right
BACK = dict(xl=-9.6, xr=7.4, y0=1.4, y1=8.4)          # bottom arrow, points left

icon("converse", "subject and predicate trade places: the arrow comes back the other way (חלוף כולל)",
     tile(GO["xl"], GO["xr"], GO["y0"], GO["y1"], V, single="square") + "\n  " +
     tile(BACK["xl"], BACK["xr"], BACK["y0"], BACK["y1"], V, flip=True, single="circle"), V)

icon("converse-limited", "trade places, but it only comes back for some: a shorter return arrow (חלוף קצתי)",
     tile(GO["xl"], GO["xr"], GO["y0"], GO["y1"], V, single="square") + "\n  " +
     tile(0.4, 9.6, 2.6, 7.6, V, flip=True, single="circle", barb=0.7), V)

icon("contrapositive", "trade places, and is becomes is not: the return arrow in negative (חלוף הפכי)",
     tile(GO["xl"], GO["xr"], GO["y0"], GO["y1"], V, single="square") + "\n  " +
     tile(BACK["xl"], BACK["xr"], BACK["y0"], BACK["y1"], V, flip=True, single="circle", inverted=True), V)
