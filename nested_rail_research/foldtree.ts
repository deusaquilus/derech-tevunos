/**
 * Reference model for nested folds — a candidate replacement for the `Focus`
 * half of `viz/src/folding.ts`. Research artefact: `viz/src` is imported
 * read-only and nothing here is wired into the app.
 *
 * The idea in one paragraph. v3 keeps one `Focus {id, folded}` and derives one
 * band from it; a new long-reaching arrival replaces the focus, so the band it
 * derived is lost ("released"). Here the state is a *laminar family of bands*
 * — closed or open intervals of rows, each hanging under an *anchor*, the unit
 * whose teeth it hides — plus the reader's *attention*, the relation whose
 ...
 * state from scratch exactly as v3's `focusAfterSeek` does.
 */

import type { Unit } from "../viz/src/sugya.ts";
import { arrivesFolded, landingStart, reachOf } from "../viz/src/folding.ts";

// --- policy ------------------------------------------------------------------

export type Policy = {
  /** v3's `LONG_REACH`: sentences between a move and its target before it counts as long-reaching. */
...
const descends = (tree: Tree, i: number, ancestor: number): boolean => ancestorsOf(tree, i).includes(ancestor);
...
const keyOf = (kind: Band["kind"], anchor: number, from: number): string => `${kind}:${anchor}:${from}`;

const contains = (outer: Band, inner: Band): boolean => outer.from <= inner.from && inner.to <= outer.to;
 ...
 * the eleven sugyot; the rule exists so the invariant is unconditional.
 */
export const insert = (bands: readonly Band[], band: Band): Band[] => {
  const kept = bands.flatMap((x): Band[] => {
    ...
    if (contains(band, x)) {
      const extends_ = x.kind === "frame" && band.kind === "frame" && x.anchor === band.anchor;
      return extends_ ? [] : [{ ...x, closed: true }];
    }
...
const isLong = (units: readonly Unit[], i: number, policy: Policy): boolean => reachOf(units, i) >= policy.longReach;
...
const itemsOf = (tree: Tree, bands: readonly Band[], from: number, to: number): Item[] => {
  const blocks = sortBands(
    bands.filter((b) => b.closed && from <= b.from && b.to <= to &&
      !bands.some((o) => o !== b && o.closed && from <= o.from && o.to <= to && contains(o, b) && !same(o, b))),
  );
  const items: Item[] = [];
  let i = from;
  while (i <= to) {
...
const lineOf = (tree: Tree, item: Item): number[] =>
  item.row !== undefined ? ancestorsOf(tree, item.row) : item.parent === undefined ? [] : ancestorsOf(tree, item.parent);
...
const anchorOfItems = (tree: Tree, run: readonly Item[], fallback: number): number => {
  ...
  const outside = [...new Set(run.flatMap((it) => (it.parent !== undefined && (it.parent < first.from || it.parent > last.to) ? [it.parent] : [])))];
 ...
 * the same rule relative to its own anchor.
 */
const decomposeItems = (
  ...
  const isTooth = (it: Item): boolean => it.parent === anchor || (it.row !== undefined && isLong(units, it.row, policy));
  ...
  const span = (run: readonly Item[]): { from: number; to: number } => ({ from: run[0]!.from, to: run[run.length - 1]!.to });
  ...
  while (i < items.length) {
    ...
    let j = i;
    while (j + 1 < items.length && !isTooth(items[j + 1]!)) j += 1;
...
/** Convenience over rows alone, for callers with no stored bands. */
export const decompose = (units: readonly Unit[], tree: Tree, anchor: number, from: number, to: number, policy: Policy): Band[] =>
  decomposeItems(units, tree, anchor, itemsOf(tree, [], from, to), policy, { from, to });
...
const decomposeAround = (
  ...
  const leaves = decomposeItems(units, tree, anchor, itemsOf(tree, bands, from, to), policy, { from, to });
  return leaves.reduce<Band[]>((acc, leaf) => (acc.some((b) => same(b, leaf)) ? acc : insert(acc, leaf)), [...bands]);
};

// --- transitions -----------------------------------------------------------------

const outermostClosedOver = (bands: readonly Band[], i: number): Band | undefined =>
  bands.find((b) => b.closed && covers(b, i) && !bands.some((o) => o !== b && o.closed && covers(o, i) && contains(o, b) && !same(o, b)));

/**
 * Open a band one level: what it already remembers stays closed inside it,
 * and the gaps between are decomposed.
 */
const openOne = (units: readonly Unit[], tree: Tree, bands: readonly Band[], band: Band, policy: Policy): Band[] => {
...
export const expose = (units: readonly Unit[], tree: Tree, bands: readonly Band[], target: number, policy: Policy): readonly Band[] => {
 ...
 */
export const arrive = (units: readonly Unit[], tree: Tree, state: FoldState, policy: Policy): FoldState => {
...
export const replay = (units: readonly Unit[], revealed: number, policy: Policy = DEFAULT_POLICY): FoldState => {
  const tree = treeOf(units);
  let state = EMPTY;
  for (let i = 0; i < Math.min(revealed, units.length); i += 1) state = arrive(units, tree, state, policy);
  return state;
};
 ...
 * opening is again one level.
 */
export const toggle = (units: readonly Unit[], state: FoldState, key: string, policy: Policy = DEFAULT_POLICY): FoldState => {
  const tree = treeOf(units);
  const band = state.bands.find((b) => b.key === key);
  if (band === undefined) return state;
  // Nothing to press on a band hidden inside a closed one.
  if (state.bands.some((o) => o !== band && o.closed && contains(o, band) && !same(o, band))) return state;
  if (band.closed) {
    const bands = openOne(units, tree, state.bands, band, policy);
    const slots = layout(state.revealed, bands);
    const onAnchor = units
      .map((_, i) => i)
      .filter((i) => i < state.revealed && i >= band.from && tree.parent[i] === band.anchor && isLong(units, i, policy) && isRow(slots, i));
    const attention = onAnchor.length > 0 ? onAnchor[onAnchor.length - 1] : state.attention;
    return { ...state, bands, attention };
  }
  const bands = state.bands.map((b) => (b.key === key ? { ...b, closed: true } : contains(band, b) ? { ...b, closed: true } : b));
  return { ...state, bands };
};
 ...
 */
export const press = (units: readonly Unit[], state: FoldState, m: number, policy: Policy = DEFAULT_POLICY): FoldState => {
  const tree = treeOf(units);
  ...
  if (existing !== undefined) {
    ...
    const bands = exposed.map((b) => (b.key === existing.key || (contains(existing, b) && !same(existing, b)) ? { ...b, closed: true } : b));
...
export const layout = (rendered: number, bands: readonly Band[]): Slot[] => {
  ...
  let i = 0;
  while (i < rendered) {
    const band = bands.find((b) => b.closed && b.from === i && !bands.some((o) => o !== b && o.closed && contains(o, b) && !same(o, b)));
    if (band !== undefined) {
...
export const isRow = (slots: readonly Slot[], index: number): boolean => slots[slotOf(slots, index)]?.kind === "row";
 ...
 */
export const railsOf = (units: readonly Unit[], state: FoldState, slots: readonly Slot[], policy: Policy = DEFAULT_POLICY): Rail[] => {
  const tree = treeOf(units);
  ...
  const railFor = (anchor: number, move: number): Rail => {
    const between = units
      .map((_, i) => i)
      .filter((i) => i > anchor && i < move && i < state.revealed && isRow(slots, i) && tree.parent[i] === anchor);
...
export const assertLaminar = (bands: readonly Band[], where: string): void => {
  for (const a of bands) {
    for (const b of bands) {
      ...
      throw new Error(`${where}: bands cross — ${a.key} [${a.from}..${a.to}] vs ${b.key} [${b.from}..${b.to}]`);
    }
  }
};
