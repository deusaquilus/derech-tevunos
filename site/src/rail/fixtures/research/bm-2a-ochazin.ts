...
export const bavaMetziaOchazin = skeleton(
  ...
  [
    ["mishnah", "S", undefined, "שנים אוחזין בטלית… ויחלוקו", "Mishnah: two hold a garment, each says I found it, each says it is all mine", { speaker: "Mishnah", anatomy: [{ kind: "compound-equal", note: "Read as Rav Pappa reads it: `רישא במציאה וסיפא במקח וממכר`, one predicate — they divide — of two subjects, the finders and the buyers, which is Ramchal's `נשוא אחד בשני נושאים` (Heb p33). On his first branch, `בהשואה אחת`, because the Gemara's own `וצריכא` shows that neither clause is implied by the other: had it taught only the find, or only the sale, the other would not follow. That is the test that separates equal footing from his known-and-novel branch, where one part is `שכבר נודע` and the Gemara would say `לא זו אף זו קתני`. The first answer, `חדא קתני`, read the two clauses as one case and was refuted by `זה וזה קתני`. The two commitments are indexed in the English; the Hebrew is an incipit and the clauses fall inside its `…`. `תינח מציאה, מקח וממכר מאי איכא למימר` later lands on the second alone." }], spans: { commitment: [{ en: [{ from: 6, to: 10 }], note: "the finders' clause — Rav Pappa's `רישא במציאה`" }, { en: [{ from: 11, to: 16 }], note: "the buyers' clause — `סיפא במקח וממכר`; the `וצריכא` shows neither follows from the other" }] } }],
    ...
    ["a-reisiha", "R", "q-metzasiha", "אי תנא אני מצאתיה הוה אמינא ראיתיה", "'Found' alone could mean 'saw'; 'all mine' rules that out", { marker: "אי תנא … הוה אמינא", anatomy: [{ kind: "hypothetical", basis: "marked", note: "`אי תנא … הוה אמינא`: had the Mishnah taught only this, one would have read it so — only the dependence is asserted, and `q-rabbanai` attacks its consequent." }], spans: { antecedent: [{ he: [{ from: 1, to: 4 }], en: [{ from: 1, to: 2 }], note: "the condition: had the Mishnah taught only 'I found it'" }], consequent: [{ he: [{ from: 5, to: 7 }], en: [{ from: 3, to: 5 }], note: "what would have followed: 'found' could mean 'saw' — the reading `q-rabbanai` attacks" }] } }],
    ...
    ["q-chenvani", "D", "a-yose-1", "בין לרבנן ובין לרבי יוסי, התם גבי חנוני על פנקסו", "For rabbis and R. Yose alike: the shopkeeper's ledger — both swear and collect, though a cheat is certain", { anatomy: [{ kind: "compound-equal", note: "`בין לרבנן ובין לרבי יוסי`: one difficulty said of two views in one breath, the rabbis' and R. Yose's, and said of them alike — Ramchal's one predicate of several subjects `בהשואה אחת` (Heb p33). The rail can draw one target; the label says the difficulty reaches both sides. No spans: the two subjects share every word of the predicate, so there is nothing to cut." }] }],
